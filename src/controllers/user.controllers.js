...
import { uploadBufferToCloudinary } from "../config/multer.js";
...
export const checkUsernameExists = asyncHandler(async (req, res, next) => {
  ...
  res.status(200).json(
    new ApiResponse(200, "Username check completed", {
      exists: !!existingUser,
...
export const checkEmailExists = asyncHandler(async (req, res, next) => {
  ...
  res.status(200).json(
    new ApiResponse(200, "Email check completed", {
      exists: !!existingUser,
      available: !existingUser,
...
export const registerUser = asyncHandler(async (req, res, next) => {
  ...
  return res
    .status(201)
    .json(
      new ApiResponse(201, "User registered successfully", { id: user._id })
    );
});
...
export const registerGoogleUser = asyncHandler(async (req, res, next) => {
  ...
  const user = await User.create({
    ...
    loggedInVia: "google",
    profilePicture:
      profilePicture || profilePictures[userCount % profilePictures.length],
...
export const loginUser = asyncHandler(async (req, res, next) => {
  ...
  if (user.loggedInVia === "google") {
    return next(
      new ApiError(
        400,
        "Account created via Google. Please login using Google."
      )
    );
  }
...
export const userDetails = asyncHandler(async (req, res, next) => {
  ...
  const followers = await FriendRequest.find({ receiver: userId }).select(
    "sender"
  );
  ...
  const following = await FriendRequest.find({ sender: userId }).select(
    "receiver"
  );
...
export const updateUserAvatar = asyncHandler(async (req, res, next) => {
  ...
  // If a file was uploaded via multer memoryStorage, upload the buffer to Cloudinary
  let finalUrl = null;
  ...
  if (req.file?.buffer) {
    try {
      ...
      finalUrl = result?.secure_url || null;
    } catch (err) {
      return next(new ApiError(500, `Image upload failed: ${err.message}`));
    }
  } else {
    // Fallback to URL provided in request body
    finalUrl = req.body?.profilePicture || null;
  }
...
export const updateUserCover = asyncHandler(async (req, res, next) => {
  ...
  // 1️⃣ If a file is uploaded (via multer memoryStorage)
  if (req.file?.buffer) {
    try {
      const result = await uploadBufferToCloudinary(
        req.file.buffer,
    ...
    // 2️⃣ Fallback to URL provided in request body
    finalUrl = req.body?.coverImage || null;
  }

  // 3️⃣ Validate final URL
  if (!finalUrl) {
    return next(new ApiError(400, "No image provided"));
  }
