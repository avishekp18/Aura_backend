...
export const sendFriendRequest = asyncHandler(async (req, res, next) => {
  ...
  res
    .status(201)
    .json(
      new ApiResponse(201, "Friend request sent successfully", friendRequest)
    );
});
...
export const getSentRequests = asyncHandler(async (req, res, next) => {
  ...
  const sentRequests = await FriendRequest.find({ sender: userId }).populate(
    "receiver",
    "fullName username bio profilePicture coverImage"
  );
...
export const getReceivedRequests = asyncHandler(async (req, res, next) => {
  ...
  const receivedRequests = await FriendRequest.find({
    receiver: userId,
  }).populate("sender", "fullName username bio profilePicture coverImage");
  res
    .status(200)
    .json(
      new ApiResponse(
...
export const getFriendsList = asyncHandler(async (req, res, next) => {
  ...
  const user = await User.findById(userId).populate(
    "friends",
    "fullName username bio profilePicture coverImage email"
  );
  if (!user) {
  ...
  res
    .status(200)
    .json(
      new ApiResponse(200, "Friends list fetched successfully", user.friends)
    );
});
...
export const cancelSentRequest = asyncHandler(async (req, res, next) => {
  ...
  if (!friendRequest) {
    return next(
      new ApiError(404, "Friend request not found or already cancelled")
    );
  }
